...
def team_hierarchy_view(request):
    ...
    total_leads = Team.objects.filter(lead__isnull=False).exclude(lead__username='aman').values('lead').distinct().count()
...
def team_detail_view(request, pk):
    ...
    memberships = team.memberships.exclude(user__username='aman').select_related('user', 'reporting_to').all()
    ...
    all_users = User.objects.filter(is_active=True).exclude(username='aman').exclude(team_memberships__team=team).order_by('first_name', 'username')
...
def team_member_add_view(request, pk):
    ...
    if user.username == 'aman':
        messages.error(request, "Master user 'aman' is reserved for website administration only and cannot be assigned to teams.")
        return redirect('teams:team_detail', pk=team.pk)

    reporting_to = User.objects.filter(pk=reporting_to_id).exclude(username='aman').first() if reporting_to_id else None
